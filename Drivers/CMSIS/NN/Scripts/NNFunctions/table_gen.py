...
class Table(object):
  ...
  def fp2q7(self, x):
    ...
    if x_int >= 128 :
      x_int = 127
    x_int = max(x_int, -128)
    return x_int if x_int >= 0 else 0x100 + x_int
  
  def fp2q15(self, x):
    x_int = math.floor(x*(2**15)+0.5)
    if x_int >= 2**15 :
      x_int = 2**15-1
    x_int = max(x_int, -1*2**15)
    return x_int if x_int >= 0 else 0x10000 + x_int

  def table_gen(self):
    with open("NNCommonTable.c", "wb") as outfile:
      outfile.write("/*\n * Common tables for NN\n *\n *\n *\n *\n */\n\n#include \"arm_math.h\"\n#include \"NNCommonTable.h\"\n\n/*\n * Table for sigmoid\n */\n")

      for function_type in ["sigmoid", "tanh"]:
        for data_type in [7, 15]:
          out_type = f"q{str(data_type)}_t"
          act_func = getattr(self, function_type)
          quan_func = getattr(self, f'fp2q{str(data_type)}')
